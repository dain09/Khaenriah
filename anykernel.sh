    ...
    while true; do
        ...
        if dumpsys power | grep -q "mWakefulness=Asleep"; then
            if ! $screen_is_off; then
                log_event "Screen OFF. Deep Sleep activated."
                # MODIFIED: Use a specific ID for the deep sleep notification so it can be removed upon waking.
                if [ "$(cat "$MODDIR/notifications.conf" 2>/dev/null)" = "true" ]; then
                    TERMUX_CMD_ON="PATH=/data/data/com.termux/files/usr/bin:\$PATH /data/data/com.termux/files/usr/bin/termux-notification --title \"Deep Sleep\" --content \"Enhanced power saving activated.\" --id khaenriah-deepsleep-status"
            ...
            if $screen_is_off; then
                ...
                $APPLY_SCRIPT
                # MODIFIED: Remove the persistent "activated" notification, then show the temporary "exited" notification.
                (
                    # Remove the notification that was shown when the screen went off
                    su -c "PATH=/data/data/com.termux/files/usr/bin:\$PATH /data/data/com.termux/files/usr/bin/termux-notification-remove khaenriah-deepsleep-status" >/dev/null 2>&1
                    
                    # Send a temporary notification that disappears after 5 seconds
                    TERMUX_CMD_OFF="PATH=/data/data/com.termux/files/usr/bin:\$PATH /data/data/com.termux/files/usr/bin/termux-notification --title \"Deep Sleep\" --content \"Exited power saving mode.\" --id khaenriah-temp"
